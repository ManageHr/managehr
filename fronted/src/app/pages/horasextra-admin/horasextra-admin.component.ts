...
export class HorasextraAdminComponent {
  ...
  ngOnInit(): void {
    ...
    console.log('Horasextra', this.horasextras); 
  }
  ...
  const datosAgrupados = this.horasextras.reduce((acc: any, inc: any) => {
    const doc = inc.contrato?.hoja_de_vida?.usuario?.numDocumento || 'Sin documento';
  ...
  new Chart('graficoArea', {
    ...
    this.horasextraService.obtenerTodas().subscribe({
      next: (res: Horasextra[]) => {
        console.log('horasextra', res); // debe mostrar el array
   ...
   eliminarhorasextra(id: number): void {
    this.horasextraService.eliminar(id).subscribe({
      next: () => {
        this.horasextras = this.horasextras.filter(i => i.idHorasExtra !== id);
      ...
      error: (err) => {
        console.error('Error al eliminar la horasextra', err);
        Swal.fire({
          icon: 'error',
          title: 'Error',
  ...
  descargarPDFPorUsuario() {
    ...
    imgLogo.onload = () => {
      ...
      const agrupadoPorUsuario = this.horasextrasFiltradas.reduce((acc: any, inc: any) => {
  ...
  descargarExcelPorUsuario() {
    ...
    const horasextrasAgrupadas = this.horasextrasFiltradas.reduce((acc: any, inc: any) => {
    ...
    Object.values(horasextrasAgrupadas).forEach(({ usuario, registros }: any) => {
      ...
      const encabezadoRow = sheet.addRow([
        'Documento', 'Área', 'Tipo de Hora', 'Cantidad de Horas'
      ]);
  ...
  descargarPDFPorArea() {
    ...
    imgLogo.onload = () => {
      ...
      Object.entries(agrupadoPorArea).forEach(([area, lista]: any) => {
        ...
        autoTable(doc, {
          head: [['Documento', 'Nombre Usuario', 'Tipo de Hora', 'Cantidad de Horas']],
          body,
          startY: startY + 5,
          theme: 'grid',
          styles: { halign: 'left', fontSize: 10 },
      ...
      const numDocumento = this.horasextras?.[0]?.contrato?.hoja_de_vida?.usuario?.numDocumento || 'sin_documento';
  ...
  descargarExcelPorArea() {
    ...
    Object.entries(agrupadoPorArea).forEach(([area, registros]: any) => {
      ...
      const encabezadoRow = sheet.addRow([
        'Documento', 'Nombre Usuario', 'Tipo de Hora', 'Cantidad de Horas'
      ]);
      ...
      registros.forEach((inc: any, index: number) => {
        const u = inc.contrato?.hoja_de_vida?.usuario || {};
        const tipo = inc.tipo_hora_extra?.nombreTipoHoras || 'N/A';
        const nombreCompleto = `${u.primerNombre || ''} ${u.primerApellido || ''}`.trim();
