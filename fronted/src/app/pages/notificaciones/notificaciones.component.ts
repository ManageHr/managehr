...
export class NotificacionesComponent implements OnInit {
  ...
  filtrarNotificacionesParaEmpleado(todas: Notificacion[]): void {
    ...
    if (!contratoIdUsuario) {
      console.error('❌ No se puede filtrar: contratoId del empleado no encontrado');
      console.log('🔍 Usuario completo para debug:', this.usuario);
      Swal.fire('Info', 'No se encontró información de contrato asociada a tu usuario', 'info');
      return;
    }
    ...
    if (this.notificacionesPendientes.length > 0) {
      ...
      console.log('🔍 Buscando notificaciones con contratoId:', contratoIdUsuario);
      const notificacionesDelContrato = todas.filter(n =>
        n.contratoId !== null &&
        n.contratoId === contratoIdUsuario.toString()
      );
      console.log('📝 Notificaciones con este contratoId:', notificacionesDelContrato.length);
  ...
  private getContratoIdUsuario(): number | null {
    ...
    // 2. Buscar en usuario.perfil.contratoId
      ...
      console.log('✅ ContratoId encontrado en usuario.perfil.contratoId:', contratoId);
    }
    // 3. Buscar en usuario.contrato.idContrato
      ...
      console.log('✅ ContratoId encontrado en usuario.contrato.idContrato:', contratoId);
    }
    // 4. Buscar en usuario.id (como fallback)
      ...
      console.log('⚠️ Usando id del usuario como contratoId (fallback):', contratoId);
    }

    if (!contratoId) {
      console.error('❌ No se encontró contratoId en el usuario:', this.usuario);
      // Mostrar estructura completa del usuario para debug
      console.log('🔍 Estructura completa del usuario:', JSON.stringify(this.usuario, null, 2));
    }
  ...
  marcarComoVista(notificacion: Notificacion): void {
    ...
    this.notificacionesService.actualizarEstado(notificacion.idNotificacion, 1).subscribe({
      next: (response) => {
        console.log('✅ Notificación marcada como vista:', notificacion.idNotificacion);
        // Actualizar la lista local
        notificacion.estado = "1";
        // Remover de pendientes y agregar a aceptadas
        this.notificacionesPendientes = this.notificacionesPendientes.filter(n => n.idNotificacion !== notificacion.idNotificacion);
  ...
  getHastaItem(): number {
    return Math.min(this.paginaPendientes * this.itemsPerPage, this.notificacionesPendientesFiltradas.length);
  }
  ...
  getHastaItemAceptadas(): number {
    return Math.min(this.paginaAceptadas * this.itemsPorPaginaAceptadas, this.notificacionesAceptadasFiltradas.length);
  }
